...
const ProductsPage = () => {
  ...
  React.useEffect(() => {
    import('../../services/cmsService').then(({ cmsService }) => {
      cmsService.getProducts().then(dyn => {
        const published = dyn.filter(p => p.status === 'published');
        const formatted = published.map(p => ({
          id: `dyn-${p.id}`,
          title: p.title,
          description: p.description || p.features || 'Explore our newest product.',
          image: p.image_url || sahyakAIImg,
          route: `/products/${p.slug}`
        }));
        setProductsList(prev => {
          const newItems = formatted.filter(n => !prev.some(p => p.route === n.route));

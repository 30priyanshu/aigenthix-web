...
const Services = () => {
  ...
  React.useEffect(() => {
    import('../../services/cmsService').then(({ cmsService }) => {
      cmsService.getServices().then(dyn => {
        const published = dyn.filter(s => s.status === 'published');
        const formatted = published.map(s => ({
          slug: s.slug,
          title: s.title,
          points: [s.description || 'Custom solutions tailored to your needs', s.benefits || 'Innovative and scalable architecture'],
          audience: 'Various Industries',
          image: s.icon_url || s.image_url || generativeAIImg
        }));

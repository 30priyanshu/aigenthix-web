...
const Industries = () => {
  ...
  const [industriesList, setIndustriesList] = useState(industriesData);

  useEffect(() => {
    import('../../services/cmsService').then(({ cmsService }) => {
      cmsService.getIndustries().then(dyn => {
        const published = dyn.filter(i => i.status === 'published');
        const formatted = published.map(i => ({
          id: `dyn-${i.id}`,
          slug: i.slug,
          title: i.name || i.title,
          description: i.description || 'Innovative industry solution',
          image: i.image_url || enterpriseImg
        }));
        setIndustriesList(prev => {
          const newItems = formatted.filter(n => !prev.some(p => p.slug === n.slug));
          return [...prev, ...newItems];
        });
      }).catch(err => console.error("Error fetching dynamic industries", err));
    });
  }, []);
  ...
  const nextSlide = useCallback(() => {
    setCurrentIndex((prevIndex) =>
      prevIndex === industriesList.length - 1 ? 0 : prevIndex + 1
    );
  }, [industriesList.length]);

  const prevSlide = useCallback(() => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? industriesList.length - 1 : prevIndex - 1
    );
  }, [industriesList.length]);
        ...
        {isMobile && (
          <div className="relative mx-[-1rem] sm:mx-[-1.5rem] px-4 sm:px-6">
            ...
            <div className="text-center mt-4 text-gray-600 text-sm font-medium">
              {currentIndex + 1} / {industriesList.length}
            </div>
          </div>
        )}
      </div>

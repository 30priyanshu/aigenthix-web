...
const initialProductsDropdownItems = [
...
const initialServiceDropdownItems = [
...
const initialIndustriesDropdownItems = [
...
const Navbar = () => {
  ...
  const [productsDropdownItems, setProductsDropdownItems] = useState(initialProductsDropdownItems);
  const [serviceDropdownItems, setServiceDropdownItems] = useState(initialServiceDropdownItems);
  const [industriesDropdownItems, setIndustriesDropdownItems] = useState(initialIndustriesDropdownItems);

  React.useEffect(() => {
    const fetchDynamicNavItems = async () => {
      try {
        const [dynProducts, dynServices, dynIndustries] = await Promise.all([
          cmsService.getProducts(),
          cmsService.getServices(),
          cmsService.getIndustries()
        ]);

        const formatDyn = (items, prefix) => 
          items.filter(i => i.status === 'published').map(i => ({ title: i.title || i.name, to: `/${prefix}/${i.slug}` }));

        setProductsDropdownItems(prev => {
          const newItems = formatDyn(dynProducts, 'products').filter(n => !prev.some(p => p.to === n.to));
          return [...prev, ...newItems];
        });

        setServiceDropdownItems(prev => {
          const newItems = formatDyn(dynServices, 'services').filter(n => !prev.some(p => p.to === n.to));
          return [...prev, ...newItems];
        });

        setIndustriesDropdownItems(prev => {
          const newItems = formatDyn(dynIndustries, 'industries').filter(n => !prev.some(p => p.to === n.to));
          return [...prev, ...newItems];
        });
      } catch (err) {
        console.error("Failed to load dynamic nav items", err);
      }
    };

...
class Settings(BaseSettings):
    ...
    def parse_lists(self) -> "Settings":
        ...
        for api_key in (self.GEMINI_API_KEY, self.GEMINI_API_KEY_2):
            normalized_key = (api_key or "").strip()
            if normalized_key and normalized_key not in self._gemini_api_keys:
                self._gemini_api_keys.append(normalized_key)
        return self
